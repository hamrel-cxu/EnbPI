from scipy.stats import skew  # For me
...
# Additional: plot distribution of training errors to examine
# under-estimate or over-estimate


def training_error_hist(predictor):
    tot_trial = 1
    rnn = False
    alpha_ls = [0.05]
    for one_dim in [False]:
        for data_name in Data_name:
            data = eval(f'data{data_name}')  # Pandas DataFrame
            data_x = data.loc[:, data.columns != response_ls[data_name]]
            data_y = data[response_ls[data_name]]
            ...
            for itrial in range(tot_trial):
                ...
                for alpha in alpha_ls:
                    ...
                    if predictor == 'NN':
                        pred = nnet
                    EnbPI_pred = EnbPI.prediction_interval(
                        pred,  X_train, X_predict, Y_train, Y_predict)
                    result = EnbPI_pred.run_experiments(
                        alpha, B, stride, data_name, itrial, miss_test_idx, skewness='right', methods=methods)
    all_pred_error = EnbPI_pred.Ensemble_online_resid_non_abs
    training_pred_error = all_pred_error[:train_size]
    # Plot seaborn histogram overlaid with KDE
    plt.rcParams['font.size'] = '16'
    ax = sns.histplot(data=training_pred_error, bins=80, alpha=1, edgecolor='white',
                      linewidth=0.5, line_kws=dict(color='black', alpha=0.5, linewidth=1.5, label='KDE'))
    # ax = sns.histplot(data=training_pred_error, bins=15, stat='density', alpha=1, kde=True, edgecolor='white',linewidth=0.5,line_kws=dict(color='black', alpha=0.5, linewidth=1.5, label='KDE'))
    # Edit legemd and add title
    ax.legend(frameon=False)
    skewness = np.round(skew(training_pred_error), 2)
    print(f'Skewness is {skewness}')
    if skewness > 0:
        ax.set_title(
            f'Solar data residual histogram \n Skewness: {skewness}, skewing right', fontsize=18, pad=15)
    else:
        ax.set_title(
            f'Solar data residual histogram \n Skewness: {skewness}, skewing left', fontsize=18, pad=15)
    ax.set_xlabel('Residual Magnitude')
    plt.savefig(
        f'{predictor}_Training_error_histogram.pdf', dpi=300, bbox_inches='tight',
        pad_inches=0)
    plt.show()


training_error_hist('RF')
...
def all_together(Data_name, sub, no_slide, missing, miss_frac=0.25, one_dim=False, skewness='none'):
    methods = ['Ensemble']
    train_days = 92
    density_est = False
    itrial = 1
    ...
    for data_name in Data_name:
        ...
        print(f'regressor is {ridge_cv.__class__.__name__}')
        result_ridge = ridge_results.run_experiments(
            alpha, B, stride, data_name, itrial, miss_test_idx, skewness, methods=methods, get_plots=True)
        print(f'regressor is {random_forest.__class__.__name__}')
        result_rf = rf_results.run_experiments(
            alpha, B, stride, data_name, itrial, miss_test_idx, skewness, methods=methods, get_plots=True)
        print(f'regressor is {nnet.name}')
...
dataSolar_Atl = util.read_data(3, 'Data/Solar_Atl_data.csv', max_data_size)
# Get results
skewness = 'right'  # Based on earlier RF error histogram
importlib.reload(sys.modules['PI_class_EnbPI'])  # For me
results_ls_with_missing_and_slide_sub = all_together(
    Data_name=ATL_cities, sub=1, no_slide=False, missing=True, one_dim=False, skewness=skewness)
# Make plots
importlib.reload(sys.modules['utils_EnbPI'])  # For me
util.make_cond_plots(ATL_cities, results_ls_with_missing_and_slide_sub,
